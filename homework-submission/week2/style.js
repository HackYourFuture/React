...
const StyledButton = styled.button`
  background-color: ${props => (props.inverse ? `#ffffff` : theme.colors[props.theme].main)};
  outline: 0;
  border: 0;
  ...
  border: ${props => props.border && `1px solid ${theme.colors[props.theme].main}`};
  transition: background-color 0.2s;
  cursor: pointer;

  &:hover,
  &:focus {
    background-color: ${props => `${props.inverse ? '#ffffff' : theme.colors[props.theme].dark}`};
    color: ${props => `${props.inverse ? theme.colors[props.theme].light : '#ffffff'}`};
  }
`;
