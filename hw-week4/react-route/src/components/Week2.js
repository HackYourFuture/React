...
const Item = ({
  ...
  return (
    <div className="items">
      <div className={itemStyle} onClick={itemHandler}>
        <span id="description" className={descriptionStyle}>
          {description}
        </span>
...
class Week2 extends React.Component {
  ...
  itemDone = index => {
    const item = this.state.todo[index];
        ...
        <Todo
          ...
          items={this.state.todo.map((item, index) => (
            <Item
              ...
              itemHandler={() => this.itemDone(index)}
              removeHandler={() => this.removeItem(index)}
              itemStyle={`item ${this.state.todo[index].done ? 'font-color' : 'bg-color-hover'}`}
              descriptionStyle={`block ${this.state.todo[index].done && 'through'}`}
            />
          ))}
        />
      </div>
